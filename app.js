...
const FLAT_EQUIV  = { 'A#':'Bb','D#':'Eb','G#':'Ab' };
...
function chordFileMajor_JS(key, CA) {
  ...
  const sevenRoot   = transpose(key, -1, preferFlats);
  const SevenDim    = sevenRoot ? `${sevenRoot}°` : '';
  const FiveSeven   = sevenRoot ? `${transpose(sevenRoot, 7, preferFlats)}7` : '';
...
function scoreKeyAgainstChords(key, chordsNorm) {
  ...
  for (const c of chordsNorm) {
    if (!c) continue;
    if (dia.has(c)) hit++; else if (near.has(c)) nearHit++;
  }
